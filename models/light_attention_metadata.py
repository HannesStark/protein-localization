...
class LightAttention(nn.Module):
    def __init__(self, embeddings_dim=1024, output_dim=11, dropout=0.25, kernel_size=9, conv_dropout: float = 0.25):
        ...
        self.linear = nn.Sequential(
            nn.Linear(2 * embeddings_dim + 26, 32),
            nn.Dropout(dropout),
            nn.ReLU(),
            nn.BatchNorm1d(32)
        )
    ...
    def forward(self, x: torch.Tensor, mask, sequence_lengths, frequencies) -> torch.Tensor:
        ...
        Args:
            ...
            sequence_lengths: [batchsize, 1]
            frequencies: [batch_size, 25] frequency of every amino acid
        ...
        o = torch.cat([o1, o2, sequence_lengths, frequencies], dim=-1)  # [batchsize, 2*embeddings_dim + 26]
        o = self.id2(o)
