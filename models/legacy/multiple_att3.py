...
class MultipleAtt3(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask, sequence_lengths, frequencies) -> torch.Tensor:
        ...
        o = self.conv3(x)  # [batch_size, embeddings_dim, sequence_length]
        o = self.dropout3(o)  # [batch_size, embeddings_dim, sequence_length]
        attention = self.attend3(x)
        attention = attention.masked_fill(mask[:, None, :] == False, -1e9)
        o3 = torch.sum(o * self.softmax(attention), dim=-1)  # [batchsize, embeddingsdim]

        o4, _ = torch.max(o, dim=-1)
